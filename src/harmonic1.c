...
/*  Read input data */
    ...
    printf("Max value of x (typical value: 10) > ");
    scanf("%lf",&xmax);
    printf("Number of grid points (typically a few hundreds) > " );
    scanf("%d",&mesh);
...
/*  set up the potential (must be even w.r.t. x=0) */
    ...
    for (;;) {
      ...
      printf("Number of nodes (type -1= to stop) > ");
      scanf("%d",&nodes);
      if (nodes < 0) {
	...
	e = 0.5 * (elw + eup);
	n_iter = 1000;
      } else {	   /*  test a single energy value (no bisection) */
	n_iter = 1;
      }
	...
	*/
	...
	if (icl >= mesh - 2) {
          fprintf(stderr, "Error: last change of sign too far");
          exit(1);
	}
	if (icl < 1) {
          fprintf(stderr, "Error: no classical turning point");
          exit(1);
	}
	...
	if (n_iter > 1) {
	    ...
	    if ( kkk == 1) {
	      printf("Bisection         Energy       Nodes  Discontinuity\n");
	    }
	    printf("%5d%25.15e%5d\n", kkk, e, ncross);
