...
+ Dado que \( f \) se utiliza como denominador en el metodo de Numerov, debemos garantizar que ningun elemento sea exactamente cero. Utilizamos la funcion \( np.where(condition, x, y) \):
